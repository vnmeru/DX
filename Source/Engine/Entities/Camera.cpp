...
void Camera::Update()
{
	...
	const auto camPosition = XMVectorSet( m_Transform.Position.x, m_Transform.Position.y, m_Transform.Position.z, 0.0f );
...
{
   m_Transform.Position = XMFLOAT4(0.0f, 0.f, -0.f, 0.0f);
...
void Camera::OnKeyHold(char Code)
{
    ...
    XMVECTOR posVec = XMLoadFloat4(&m_Transform.Position);
    ...
    XMStoreFloat4(&m_Transform.Position, posVec);
}
