...
// 2.1: Find a compatible graphics adapter
	...
	m_TextureManager.CreateTexture("white.jpg","white");
	m_TextureManager.CreateTexture("sphere.jpg","sphere");
	m_TextureManager.CreateTexture("oasis_Dirt.png", "oasis_Dirt");
	m_TextureManager.CreateTexture("oasis_PalmTree.png", "oasis_PalmTree");
	m_TextureManager.CreateTexture("oasis_Rock.png", "oasis_Rock");
	m_TextureManager.CreateTexture("oasis_Sand.png", "oasis_Sand");
	m_TextureManager.CreateTexture("oasis_Water.png", "oasis_Water");
...
void DirectX12::UpdatePipeline()
{
	...
	ColorHelper::RGB ConvertedColor = ColorHelper::HSV2RGB(Game::GetInstance().BGColorHSV);
	...
	for(const auto& state: m_RenderStateManager.GetPipelineStates())
	{
		m_pCommandList->SetPipelineState(state.second);
		for (auto* Entity : EManger.GetEntites())
		{
			if (Entity->GetModel() && Entity->GetModel()->RenderMode == state.first)
			{
				Entity->GetModel()->Prepare(m_pCommandList);
				
				m_pCommandList->IASetVertexBuffers(0, 1, &Entity->GetModel()->vertexBufferView); // set the vertex buffer (using the vertex buffer view)
				m_pCommandList->IASetIndexBuffer(&Entity->GetModel()->indexBufferView);

				m_pCommandList->SetGraphicsRootConstantBufferView(0, m_pConstantBufferUploadHeap[m_nFrameIndex]->GetGPUVirtualAddress() + (ConstantBufferPerObjectAlignedSize * Entity->GetModel()->MemoryOffset));

				m_TextureManager.BindTexutre(Entity->GetModel()->GetTextureName());

				m_pCommandList->DrawIndexedInstanced(Entity->GetModel()->numModelIndices, 1, 0, 0, 0);
			}
		}
	}
